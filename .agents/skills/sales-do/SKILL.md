...
**No match.** If no skill covers the need, say so and hand off: "Run: `/sales-request-skill {description}`". Do NOT ask more questions when the topic is clear but unmatched — fall through immediately.

When handing off to another router skill, your entire response is the hand-off command — no catalogs, no commentary.
